...
use windows::Win32::UI::{
    ...
    Input::{
        ...
        KeyboardAndMouse::{
            GetAsyncKeyState, SetCapture, SetFocus, TME_LEAVE, TRACKMOUSEEVENT, TrackMouseEvent,
            VIRTUAL_KEY, VK_CONTROL, VK_MENU, VK_SHIFT,
        },
    },
    WindowsAndMessaging::{
        CS_OWNDC, CallNextHookEx, CreateWindowExW, DLGC_WANTALLKEYS, DLGC_WANTCHARS,
        DefWindowProcW, DestroyWindow, GWLP_USERDATA, GetWindowLongPtrW, HHOOK, HICON, IDC_ARROW,
        LoadCursorW, MOUSEHOOKSTRUCTEX, MoveWindow, PostMessageW, RegisterClassW, SendMessageW,
        SetCursor, SetCursorPos, SetWindowLongPtrW, SetWindowsHookExW, ShowCursor,
        UnhookWindowsHookEx, UnregisterClassW, WH_MOUSE, WINDOW_EX_STYLE, WM_CHAR, WM_GETDLGCODE,
        WM_IME_CHAR, WM_IME_COMPOSITION, WM_KEYDOWN, WM_KEYUP, WM_LBUTTONDOWN, WM_LBUTTONUP,
        WM_MBUTTONDOWN, WM_MBUTTONUP, WM_MOUSEMOVE, WM_MOUSEWHEEL, WM_MOVE, WM_RBUTTONDOWN,
        WM_RBUTTONUP, WNDCLASSW, WS_CHILD, WS_VISIBLE,
    },
};
use windows::{
...
use super::{
    ...
    cursors::Cursors, drop_target::DropTarget, keyboard::virtual_key_to_keycode,
...
fn dispatch_utf16_text(window: &OsWindow, utf16: &[u16]) {
    ...
    for character in text.chars() {
        ...
        // Functional keys are handled through WM_KEYDOWN/WM_KEYUP (and their
        // forwarded WM_USER_KEY_* variants). Control characters in WM_CHAR are
        // ignored to avoid double dispatch for keys like Backspace/Delete/Enter.
...
unsafe extern "system" fn wnd_proc(
    ...
    let result = if let Some(window) = window_weak.upgrade() {
        match msg {
            WM_GETDLGCODE => LRESULT((DLGC_WANTALLKEYS | DLGC_WANTCHARS) as isize),
