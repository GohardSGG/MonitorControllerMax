use std::{ffi::OsString, os::windows::prelude::OsStrExt, str::FromStr};

use windows::Win32::{
    Foundation::HINSTANCE, System::SystemServices::IMAGE_DOS_HEADER,
...
fn to_wstr(string: impl AsRef<str>) -> Vec<u16> {
    let mut wstr: Vec<_> = OsString::from_str(string.as_ref())
        .unwrap()
        .encode_wide()
        .collect();
    wstr.push(0);
