...
impl Default for Context {
    fn default() -> Self {
        Self {
            os_window_ptr: AtomicPtr::default(),
            input_focus: AtomicU8::default(),
            keyboard_modifiers: RefCell::new(KeyboardModifiers::empty()),
        }
    }
}
...
impl OsWindowView {
    pub(crate) fn register_class() -> &'static AnyClass {
        ...
        unsafe {
            ...
            builder.add_method(
                ...
                Self::refresh_timer_fired as unsafe extern "C" fn(_, _, _) -> _,
            );
    ...
    // NSView
    unsafe extern "C" fn init_with_frame(
        this: &mut AnyObject,
        _cmd: Sel,
        rect: NSRect,
    ) -> Option<&mut AnyObject> {
        let this: Option<&mut AnyObject> =
            unsafe { msg_send![super(this, NSView::class()), initWithFrame: rect] };
        this.map(|this| {
            let ivar = this.class().instance_variable(c"_context").unwrap();
            unsafe {
                *ivar.load_mut::<Context>(this) = Context::default();
            }
            this
        })
    }
    ...
    unsafe extern "C" fn refresh_timer_fired(&self, _cmd: Sel, _timer: &NSTimer) {
        self.send_event(Event::Draw);
    }
