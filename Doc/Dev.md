...
*   **步骤 3.1：UI组件与布局**
    *   **动作：**
        ...
        4.  创建所有增益和主控旋钮/开关。
    *   **实现要点 (FlexBox 布局):**
        1.  使用 `juce::FlexBox` 来实现响应式布局，将界面垂直划分为"头部控制区"和"通道条区"。
        2.  **关键陷阱规避：** 在 `resized()` 函数中为每个通道条构建布局时，**绝不能**在循环内部使用临时的 `FlexBox` 对象 (`juce::FlexBox strip;`)。因为这些临时对象在循环结束后就会被销毁，导致 `performLayout()` 无法正确渲染。
        3.  **正确做法：** 必须将用于每个通道条的 `FlexBox` 声明为 `PluginEditor` 的**成员变量**（例如，一个 `std::array<juce::FlexBox, ...>`），以确保它们的生命周期与编辑器实例相同，从而被安全地布局。
