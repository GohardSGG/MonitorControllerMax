...
## 7.4 启动爆音 (Startup Volume Spike)

### 风险：参数恢复前的默认满音量
DAW 加载插件后，在恢复保存的参数值之前，默认参数值会短暂生效。
如果 master_gain 默认为 0 dB（满音量），可能在参数恢复前产生瞬间爆音。
- **解决方案 (v2.8.0)**:
    1. `params.rs`: master_gain 默认值改为 −∞ dB（`MINUS_INFINITY_GAIN`）。
    2. `audio.rs`: `GainSmoothingState` 初始增益改为 0.0（静音）。
    3. `channel_logic.rs`: `RenderState::default()` 所有增益改为 0.0。
    - 三层防护确保插件在任何启动路径下都从静音开始，由 DAW 恢复到保存的音量。
