...
graph TD
    ...
    Lib --> Reactor
    AudioProc -.->|AtomicCell Read| Params
...
| **mcm_reactor** | Lib | **副作用层**。基于 Tokio 的异步运行时，管理 Web 服务器和 OSC 通信。它作为插件的"Sidecar"运行。 | `mcm_core` |
...
- **交互**: 仅通过 `Atomic` 类型或 `AtomicCell`（crossbeam）读取参数和状态快照。
- **快照机制 (v2.8.0)**: 使用 `RenderSnapshot`（原子快照）代替直接锁访问。UI 线程更新快照，音频线程无锁读取，实现真正的 Lock-Free 音频处理。
...
- **组件**:
    - **Web Actor**: HTTP API / WebSocket 推送（动态分配端口）
    - **OSC Actor**: UDP 双向通信 (:7444/:7445)
    - ~~**Network Actor**: ZeroMQ 跨机广播~~ （已从模块树移除，代码待清理）
...
### 2.4.4 音频安全默认值 (v2.8.0)
- **策略**: 所有增益相关的默认值均设为 **静音 (0.0)**，而非满音量 (1.0)。
- **覆盖范围**: `MonitorParams.master_gain` (−∞ dB)、`GainSmoothingState` (0.0)、`RenderState::default()` (0.0)。
- **理由**: DAW 加载插件时，参数恢复前的短暂时间内默认值会生效。静音默认值确保不会产生爆音。
